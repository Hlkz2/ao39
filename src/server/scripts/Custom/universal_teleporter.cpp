	...
	void MainMenu(Player *pPlayer, Creature *pCreature)
	{
		...
		switch (pPlayer->GetTeam()) {
			case ALLIANCE:
				...
				pPlayer->ADD_GOSSIP_ITEM(9, "[PvP sauvage] Cratère d'Azshara", GOSSIP_SENDER_MAIN, 12);
			break;
			case HORDE:
				pPlayer->ADD_GOSSIP_ITEM(6, "[Shop] Karazhan", GOSSIP_SENDER_MAIN, 11);
				pPlayer->ADD_GOSSIP_ITEM(9, "[PvP sauvage] Cratère d'Azshara", GOSSIP_SENDER_MAIN, 13);
			break; }
				...
				pPlayer->ADD_GOSSIP_ITEM(6, "Dalaran (accès au monde...)", GOSSIP_SENDER_MAIN, 14);
